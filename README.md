...
- 글 풀은 현재 57개(꿈해몽 1개 추가)입니다. 현재 계정 수는 6개(account4, 7, 9, 10은 Meta 관련 조치로 임시 제외 중)이며, 글 풀은 계정 수보다 항상 많아야 매 실행 무중복이 보장되니 계정을 다시 늘릴 때 글도 함께 확인해주세요.
- 지원금·정책 정보 외에 라이프스타일성 콘텐츠(꿈해몽 등)도 같은 글 풀에 섞여 있습니다. 이런 콘텐츠는 민간 속설·전통 해석이라는 점과 과학적 근거가 없다는 점을 본문에 명시하는 방식으로 작성했습니다. 사실 정보(정부 지원금 등)와 오락성 정보가 한 계정에서 같이 나가는 구조이므로, 새로 추가할 때도 이 둘을 명확히 구분해서 쓰는 게 좋습니다.
...
- **더 뚜렷한 그라데이션 배경** (위→아래로 톤 변화, 기존보다 입체감 있음)
- **제목에 강조 밑줄 바** 추가 (제목 시작 부분에 포인트 색상 바)
- **번호 원형 배지** — 기존의 밋밋한 점(●) 대신, 카테고리 색상의 원 안에 굵은 흰 숫자(1, 2, 3)를 넣어 리스트 항목이 한눈에 들어오게 함
- **흰색 패널에 그림자 효과** — 패널이 배경 위에 살짝 떠 있는 것처럼 보이도록 어두운 사본을 살짝 오프셋해서 깔아둠
- **하단 CTA 바** — Threads/인스타그램 카드뉴스에서 흔히 쓰이는 "자세히 보기 →" 형태의 흰색 바 + 화살표 아이콘을 맨 아래에 추가해 클릭(다음 정보를 보고 싶게) 유도

새 글을 추가할 때도 `CARD_DATA`에 항목만 추가하면 이 디자인이 그대로 적용됩니다.
